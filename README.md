...
- **The feedback loops** (v3): every advance produces a **training report** with per-attribute gains; scouting levels unlock **actionable draft intel** (recommended ban, comp read + counter, weakness flags) and the post-match **prep report** tells you whether the homework paid; each season seeds an **Academy Showcase** (hyped in advance, revealed on the day), **breakout/slump events** for your roster, and offseason **transfer rumors** that actually come true. Design rationale in `docs/RESEARCH-loop-addictiveness.md`.
